...
class RuntimeInstallerTests(unittest.TestCase):
    def test_skip_accelerator_probe_keeps_os_arch_validation(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                detected.accelerator_detail,
                "accelerator probe skipped for requested platform",
            )
    ...
    def test_release_has_platform_asset_matches_runtime_archives_only(self) -> None:
        app_release = {
            "assets": [
                {"name": "trapo-workbench-linux-x64-v0.0.30.tar.gz"},
        ...
        self.assertTrue(
            install_runtime.release_has_platform_asset(
                REPO_ROOT, runtime_release, "linux-arm64-cpu"
            )
        )
