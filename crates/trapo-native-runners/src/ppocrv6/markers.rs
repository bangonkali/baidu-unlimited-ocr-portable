...
pub(crate) fn native_json_to_marker_text(native_json: &str) -> Result<String, String> {
...
fn append_page_lines(page: &Value, output: &mut Vec<String>) {
    ...
    for line in lines {
        ...
        output.push(format!(
            ...
            marker_safe(text),
            line_geometry(line, width, height).map_or_else(String::new, |geometry| {
                format!("<|geom|>{geometry}<|/geom|>")
            }),
            text
        ));
    }
...
fn line_geometry(line: &Value, page_width: f64, page_height: f64) -> Option<String> {
    ...
    let bounds = geometry_bounds(&points)?;
    let kind = if points.len() == 4 {
        "rotated_quad"
    } else {
        "polygon"
    };
