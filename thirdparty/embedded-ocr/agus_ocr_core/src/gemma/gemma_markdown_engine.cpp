...
const GemmaTensor& FindOutput(const std::vector<GemmaTensor>& outputs,
                              const std::string& name,
                              size_t fallback) {
  for (const GemmaTensor& output : outputs) {
    if (output.name == name) {
      return output;
    }
  }
  ...
  throw std::runtime_error("Gemma ONNX output missing: " + name);
}

std::string RuntimeSummary(agus_ocr_backend_t backend, int32_t cpu_threads) {
  std::ostringstream out;
  out << "gemma-markdown onnxruntime ";
  if (backend == AGUS_OCR_BACKEND_DIRECTML) {
    out << "directml";
  } else if (backend == AGUS_OCR_BACKEND_CUDA) {
    out << "cuda";
  } else {
    out << "cpu";
  }
...
agus_ocr_backend_t GemmaVisionBackend(agus_ocr_backend_t backend) {
  return backend == AGUS_OCR_BACKEND_DIRECTML ? AGUS_OCR_BACKEND_CPU : backend;
}
...
std::vector<KvCache> InitialKvCache(const GemmaOnnxSession& decoder) {
  std::vector<KvCache> caches;
  for (const GemmaInputInfo& input : decoder.inputs()) {
    if (!StartsWith(input.name, "past_key_values.")) {
      continue;
    }
    if (input.shape.size() != 4 || input.shape[1] <= 0 ||
        input.shape[3] <= 0) {
      throw std::runtime_error("unexpected Gemma KV-cache input shape");
    }
    KvCache cache;
  ...
  if (caches.empty()) {
    throw std::runtime_error("Gemma decoder exposes no KV-cache inputs");
  }
  return caches;
}
...
GemmaMarkdownEngine::GemmaMarkdownEngine(
    ...
    : active_backend_(active_backend),
      ...
      vision_backend_(GemmaVisionBackend(active_backend_)),
      ...
      vision_session_(bundle.vision_model_path, vision_backend_, cpu_threads_,
                      enable_profiling_, "gemma_vision"),
      embed_session_(bundle.embed_model_path, active_backend_, cpu_threads_,
                     enable_profiling_, "gemma_embed"),
      decoder_session_(bundle.decoder_model_path, active_backend_, cpu_threads_,
                       enable_profiling_, "gemma_decoder"),
      random_(std::random_device{}()) {}

GemmaMarkdownEngine::~GemmaMarkdownEngine() = default;

std::string GemmaMarkdownEngine::runtime_summary() const {
  return RuntimeSummary(active_backend_, cpu_threads_);
}

std::string GemmaMarkdownEngine::BuildPrompt(
    int image_tokens,
...
GemmaMarkdownEngine::GenerationResult GemmaMarkdownEngine::Generate(
  ...
  auto vision_outputs = vision_session_.Run({
      GemmaTensor::Float("pixel_values", image.pixel_shape, image.pixel_values),
      GemmaTensor::Int64("pixel_position_ids", image.position_shape,
                         image.position_ids),
  });
  const GemmaTensor& image_features =
      FindOutput(vision_outputs, "image_features", 0);
  result.vision_ms =
      GemmaElapsedMs(vision_start, std::chrono::steady_clock::now());
  if (image_features.floats.empty() ||
      image_features.floats.size() % kHiddenSize != 0) {
    std::ostringstream message;
    message << "Gemma vision output size is invalid: floats="
  ...
  if (result.vision_tokens <= 0) {
    throw std::runtime_error("Gemma vision encoder returned no image tokens");
  }
  ...
  for (int32_t step = 0; step < max_new_tokens; ++step) {
    ...
    auto embed_outputs = embed_session_.Run({GemmaTensor::Int64(
        "input_ids", {1, seq_len}, input_ids)});
    GemmaTensor inputs_embeds = FindOutput(embed_outputs, "inputs_embeds", 0);
    GemmaTensor per_layer_inputs =
    ...
    if (step == 0) {
      ...
      for (size_t token_index = 0; token_index < input_ids.size();
        ...
        if (image_offset + kHiddenSize > image_features.floats.size()) {
          std::ostringstream message;
          message << "Gemma prompt has more image tokens than vision features: "
      ...
      if (image_feature_index != result.vision_tokens) {
        std::ostringstream message;
        message << "Gemma prompt image-token count did not match vision features: "
    ...
    std::vector<GemmaTensor> decoder_inputs;
    decoder_inputs.push_back(GemmaTensor::Float(
        "inputs_embeds", std::move(inputs_embeds.shape),
        std::move(inputs_embeds.floats)));
    decoder_inputs.push_back(GemmaTensor::Int64(
    ...
    decoder_inputs.push_back(
        GemmaTensor::Int64("num_logits_to_keep", {}, {1}));
    decoder_inputs.push_back(GemmaTensor::Float(
        "per_layer_inputs",
    ...
    for (const KvCache& cache : kv_cache) {
      decoder_inputs.push_back(
          GemmaTensor::Float(cache.input_name, cache.shape, cache.values));
    }

    auto decoder_outputs = decoder_session_.Run(decoder_inputs);
    const GemmaTensor& logits = FindOutput(decoder_outputs, "logits", 0);
    const int64_t next_token = SelectNextToken(logits.floats, temperature);
    generated.push_back(next_token);

    if (decoder_outputs.size() < kv_cache.size() + 1) {
      throw std::runtime_error("Gemma decoder returned too few KV outputs");
    }
    for (size_t i = 0; i < kv_cache.size(); ++i) {
      const GemmaTensor& present = decoder_outputs[i + 1];
  ...
  result.generation_ms =
      GemmaElapsedMs(generation_start, std::chrono::steady_clock::now());
  return result;
}
