...
Runtime storage query call sites: `59`
Migration SQL bundles: `10`
...
| DB-OCR-001 | write/read | `replace_page_ocr`, `load_snapshot` | Upserts `ocr_documents`, `document_page_ocr`; deletes stale links/annotations/regions; inserts `document_regions`, `document_region_annotations`, and `document_text_region_links`. | `reloads_page_regions_and_spans`; `core_document_page_and_metric_queries_cover_empty_updates_and_limits` | Replacing with no boxes removes stale boxes and spans. | Empty OCR boxes/spans are valid. |
| DB-ANNOT-001 | read/write | `persist_discovered_annotation_sync` | Resolves existing annotation identity; upserts `document_annotation_identities`, discovered `document_regions`, and text links before final text is known. | `persists_discovered_annotation_identity_before_text_completion` | Re-discovering the same source key does not allocate a second id. | Same source key updates label/content while preserving UUID v7. |
...
`/api/ingest/start`, `/api/ingest/runs`, `/api/ingest/metrics/recent`,
