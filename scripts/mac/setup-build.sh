...
Options:
  ...
  --skip-build                 Do not configure/build llama.cpp.
...
run_doctor() {
    ...
    local tool detail file exe path
...
if ((doctor)); then
    run_doctor "$repo_root" "$llama_dir" "$model_dir" "$build_dir" "$((1 - skip_model_download))" "$((1 - skip_build))"
    exit 0
fi

write_step "Checking required tools"
assert_tooling "$((1 - skip_model_download))" "$((1 - skip_build))"
show_tool_versions
...
printf '%q --smoke --image %q\n' "$repo_root/scripts/mac/run-demo.sh" "<path-to-test-image>"
printf '%q\n' "$repo_root/scripts/mac/run-demo.sh"
