...
#[derive(OpenApi)]
#[openapi(
    info(title = "Trapo Server API", version = "0.1.0", description = "Rust Axum API for Trapo OCR workbench."),
    paths(
        health_doc,
        status_doc,
        openapi_doc,
...
#[utoipa::path(post, path = "/api/ingest/start", tag = "ingest", request_body = IngestStartRequest, responses((status = 200, body = IngestRunRecord), (status = 400, body = ErrorPayload)))]
fn start_ingest_doc() {}
...
#[utoipa::path(post, path = "/api/ingest/runs/{run_id}/stop", tag = "ingest", params(("run_id" = String, Path)), responses((status = 200, body = IngestRunRecord), (status = 404, body = ErrorPayload)))]
fn stop_run_doc() {}
...
#[utoipa::path(post, path = "/api/models/{model_id}/select", tag = "models", params(("model_id" = String, Path)), responses((status = 200, body = ModelSelectRecord), (status = 400, body = ErrorPayload)))]
fn select_model_doc() {}

#[utoipa::path(post, path = "/api/models/{model_id}/cancel", tag = "models", params(("model_id" = String, Path)), responses((status = 200, body = ModelDownloadRecord), (status = 400, body = ErrorPayload)))]
fn cancel_model_doc() {}
