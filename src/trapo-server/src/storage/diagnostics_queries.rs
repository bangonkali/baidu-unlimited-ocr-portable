impl Repository {
    pub fn diagnostic_runs(&self, limit: u32) -> Result<Vec<DiagnosticRunRow>> {
    ...
    pub fn diagnostic_trace(
        &self,
        filter: &DiagnosticTraceFilter<'_>,
    ) -> Result<(Vec<DiagnosticSpanRow>, Vec<DiagnosticEventRow>)> {
        Ok((
            self.diagnostic_spans(filter)?,
            self.diagnostic_events(filter)?,
        ))
    }

    pub fn diagnostic_work_units(
        &self,
        run_id: Option<&str>,
        limit: u32,
        ...
        let rows = statement.query_map(
            params![run_id, run_id, i64::from(limit.clamp(1, 100_000))],
            work_unit_from_row,
        )?;
        collect_rows(rows)
    }

    pub fn diagnostic_model_leases(
        &self,
        run_id: Option<&str>,
        limit: u32,
        ...
        let rows = statement.query_map(
            params![run_id, run_id, i64::from(limit.clamp(1, 10_000))],
            lease_from_row,
        )?;
        collect_rows(rows)
    }
    ...
    fn diagnostic_spans(
        ...
        let status = filter.status.filter(|value| *value != "all");
        let query = filter.q.map(normalized_like);
        let mut statement = conn.prepare(
