...
#if defined(TRAPO_OCR_ENABLE_LLAMA_CPP)
...
const char* GenerativeBackendLabel(trapo_ocr_generative_backend_t backend) {
  switch (backend) {
...
bool BackendCompiled(trapo_ocr_generative_backend_t backend) {
  switch (backend) {
    case TRAPO_OCR_GEN_BACKEND_CPU:
      return true;
    case TRAPO_OCR_GEN_BACKEND_CUDA:
#if defined(TRAPO_OCR_LLAMA_BACKEND_CUDA)
      return true;
#else
      return false;
#endif
    case TRAPO_OCR_GEN_BACKEND_VULKAN:
#if defined(TRAPO_OCR_LLAMA_BACKEND_VULKAN)
      return true;
#else
      return false;
#endif
    case TRAPO_OCR_GEN_BACKEND_OPENCL:
#if defined(TRAPO_OCR_LLAMA_BACKEND_OPENCL)
      return true;
#else
      return false;
#endif
...
bool DeviceMatchesBackend(ggml_backend_dev_t device,
                          trapo_ocr_generative_backend_t backend) {
  if (device == nullptr) {
    return false;
  }
...
ggml_backend_dev_t FindDeviceForBackend(trapo_ocr_generative_backend_t backend) {
  if (backend == TRAPO_OCR_GEN_BACKEND_CPU) {
    return nullptr;
  }
...
LlamaVlmBackendInfo MakeBackendInfo(trapo_ocr_generative_backend_t backend) {
...
std::vector<LlamaVlmBackendInfo> BuildBackendInfos() {
  std::vector<LlamaVlmBackendInfo> infos = {
      MakeBackendInfo(TRAPO_OCR_GEN_BACKEND_CPU),
      MakeBackendInfo(TRAPO_OCR_GEN_BACKEND_CUDA),
      MakeBackendInfo(TRAPO_OCR_GEN_BACKEND_VULKAN),
      MakeBackendInfo(TRAPO_OCR_GEN_BACKEND_OPENCL),
  };
  trapo_ocr_generative_backend_t default_backend = TRAPO_OCR_GEN_BACKEND_CPU;
  for (const auto& info : infos) {
    if (info.supported && info.backend != TRAPO_OCR_GEN_BACKEND_CPU) {
...
trapo_ocr_generative_backend_t ResolveBackend(
    const trapo_ocr_runtime_options_t& runtime) {
  if (runtime.force_cpu_only != 0 ||
      runtime.generative_backend == TRAPO_OCR_GEN_BACKEND_CPU) {
    return TRAPO_OCR_GEN_BACKEND_CPU;
  }
  if (runtime.generative_backend != TRAPO_OCR_GEN_BACKEND_AUTO) {
...
class LlamaVlmRunner::Impl {
 public:
  Impl(const std::string& model_path, const std::string& mmproj_path,
    ...
    if (active_backend_ != TRAPO_OCR_GEN_BACKEND_CPU) {
    ...
    mtmd_params.use_gpu = active_backend_ != TRAPO_OCR_GEN_BACKEND_CPU;
 ...
 private:
  int threads_ = 1;
  trapo_ocr_generative_backend_t active_backend_ = TRAPO_OCR_GEN_BACKEND_CPU;
  int gpu_layers_ = 0;
