...
#if defined(TRAPO_OCR_USE_OPENCV_MOBILE) && __has_include(<opencv2/highgui/highgui.hpp>)
...
#define TRAPO_OCR_HAS_NNAPI_PROVIDER_FACTORY 1
...
struct RuntimeOptions {
  trapo_ocr_backend_t backend = TRAPO_OCR_BACKEND_AUTO;
  ...
  trapo_ocr_generative_backend_t generative_backend =
      TRAPO_OCR_GEN_BACKEND_AUTO;
  int32_t generative_gpu_layers = 0;
};

struct AcceleratorStatus {
  trapo_ocr_backend_t backend = TRAPO_OCR_BACKEND_CPU;
...
trapo_ocr_status_t fail(trapo_ocr_status_t status, const std::string& message) {
...
constexpr size_t kRequiredRunOptionsSize =
    offsetof(trapo_ocr_run_options_t, enable_source_box_estimation);
constexpr size_t kRequiredRuntimeOptionsSize =
    offsetof(trapo_ocr_runtime_options_t, force_cpu_only);

RunOptions run_options_from_c(const trapo_ocr_run_options_t& options) {
  RunOptions out;
  ...
  if (has_size(options.struct_size,
               offsetof(trapo_ocr_run_options_t,
  ...
  if (has_size(options.struct_size,
               offsetof(trapo_ocr_run_options_t, visual_token_budget) +
...
trapo_ocr_run_options_t run_options_to_c(const RunOptions& options) {
  trapo_ocr_run_options_t out{};
...
RuntimeOptions runtime_options_from_c(
    const trapo_ocr_runtime_options_t& options) {
  RuntimeOptions out;
  ...
  if (out.force_cpu_only) {
    out.backend = TRAPO_OCR_BACKEND_CPU;
    out.generative_backend = TRAPO_OCR_GEN_BACKEND_CPU;
    out.generative_gpu_layers = 0;
  }
  return out;
}

trapo_ocr_runtime_options_t runtime_options_to_c(const RuntimeOptions& options) {
  trapo_ocr_runtime_options_t out{};
  out.struct_size = sizeof(out);
  out.backend = options.backend;
  out.cpu_threads = options.cpu_threads;
...
#else
  const char* enabled = std::getenv("TRAPO_OCR_LOG");
...
std::string& backend_last_failure(trapo_ocr_backend_t backend) {
  ...
  switch (backend) {
    ...
    case TRAPO_OCR_BACKEND_XNNPACK:
      return xnnpack_failure;
    case TRAPO_OCR_BACKEND_NNAPI:
      return nnapi_failure;
    case TRAPO_OCR_BACKEND_QNN:
      return qnn_failure;
    default:
      return directml_failure;
  }
...
bool backend_is_unhealthy(trapo_ocr_backend_t backend,
...
void mark_backend_unhealthy(trapo_ocr_backend_t backend,
...
bool directml_is_unhealthy(std::string* reason = nullptr) {
  return backend_is_unhealthy(TRAPO_OCR_BACKEND_DIRECTML, reason);
}

void mark_directml_unhealthy(const std::string& reason) {
  mark_backend_unhealthy(TRAPO_OCR_BACKEND_DIRECTML, reason);
}

std::string& document_markdown_directml_last_failure() {
  static std::string failure;
  return failure;
}

std::string& document_markdown_cuda_last_failure() {
  static std::string failure;
  return failure;
}

std::string& document_markdown_backend_last_failure(trapo_ocr_backend_t backend) {
  return backend == TRAPO_OCR_BACKEND_CUDA ? document_markdown_cuda_last_failure()
                                          : document_markdown_directml_last_failure();
}

bool document_markdown_backend_is_unhealthy(trapo_ocr_backend_t backend,
                                std::string* reason = nullptr) {
  if (backend != TRAPO_OCR_BACKEND_DIRECTML &&
      backend != TRAPO_OCR_BACKEND_CUDA) {
    if (reason != nullptr) {
      reason->clear();
    }
    return false;
  ...
  std::lock_guard<std::mutex> lock(backend_health_mutex());
  const std::string& failure = document_markdown_backend_last_failure(backend);
  const bool unhealthy = !failure.empty();
  if (reason != nullptr) {
    *reason = failure;
  }
...
void mark_document_markdown_backend_unhealthy(trapo_ocr_backend_t backend,
                                  const std::string& reason) {
  if (backend != TRAPO_OCR_BACKEND_DIRECTML &&
      backend != TRAPO_OCR_BACKEND_CUDA) {
    return;
  ...
  std::lock_guard<std::mutex> lock(backend_health_mutex());
  document_markdown_backend_last_failure(backend) = reason;
}

const char* backend_label(trapo_ocr_backend_t backend) {
  switch (backend) {
    case TRAPO_OCR_BACKEND_AUTO:
      return "auto";
    case TRAPO_OCR_BACKEND_CPU:
      return "cpu";
    case TRAPO_OCR_BACKEND_XNNPACK:
      return "xnnpack";
    case TRAPO_OCR_BACKEND_COREML:
      return "coreml";
    case TRAPO_OCR_BACKEND_DIRECTML:
      return "directml";
    case TRAPO_OCR_BACKEND_WEBASSEMBLY:
      return "webassembly";
    case TRAPO_OCR_BACKEND_WEBGPU:
      return "webgpu";
    case TRAPO_OCR_BACKEND_NNAPI:
      return "nnapi";
    case TRAPO_OCR_BACKEND_QNN:
...
const char* generative_backend_label(trapo_ocr_generative_backend_t backend) {
  switch (backend) {
    case TRAPO_OCR_GEN_BACKEND_AUTO:
      return "auto";
    case TRAPO_OCR_GEN_BACKEND_CPU:
      return "cpu";
    case TRAPO_OCR_GEN_BACKEND_VULKAN:
      return "vulkan";
    case TRAPO_OCR_GEN_BACKEND_CUDA:
      return "cuda";
    case TRAPO_OCR_GEN_BACKEND_OPENCL:
      return "opencl";
  }
  return "unknown";
}
...
struct RuntimeCapabilities {
  ...
  trapo_ocr_backend_t default_backend = TRAPO_OCR_BACKEND_CPU;
...
std::string make_runtime_summary(trapo_ocr_backend_t backend,
                                 int32_t cpu_threads) {
...
std::string make_error_json(trapo_ocr_status_t status, const std::string& message,
...
trapo_ocr_result_t* allocate_result(const std::string& json) {
  auto* result = static_cast<trapo_ocr_result_t*>(
      std::calloc(1, sizeof(trapo_ocr_result_t)));
  if (result == nullptr) {
    throw std::bad_alloc();
  }
  result->struct_size = sizeof(trapo_ocr_result_t);
...
std::string backend_display_name(trapo_ocr_backend_t backend) {
  switch (backend) {
    case TRAPO_OCR_BACKEND_CPU:
      return "CPU";
    case TRAPO_OCR_BACKEND_XNNPACK:
...
AcceleratorStatus detect_document_markdown_cuda_status() {
...
void populate_generative_capabilities(RuntimeCapabilities* capabilities) {
...
#if defined(TRAPO_OCR_ENABLE_NATIVE_PIPELINE)
  for (const auto& info : trapo_ocr::LlamaVlmBackendInfos()) {
  ...
  cpu.backend = TRAPO_OCR_GEN_BACKEND_CPU;
...
RuntimeCapabilities detect_runtime_capabilities() {
  ...
  directml_status.backend = TRAPO_OCR_BACKEND_DIRECTML;
  directml_status.unavailable_reason =
      "DirectML is not available on this platform.";
#if defined(_WIN32)
  AcceleratorStatus cuda_status = detect_document_markdown_cuda_status();
#endif

#if defined(TRAPO_OCR_ENABLE_NATIVE_PIPELINE) && defined(__ANDROID__)
  ...
  nnapi.backend = TRAPO_OCR_BACKEND_NNAPI;
  nnapi.device_name =
      "Android NNAPI API " + std::to_string(api_level);
  nnapi.supported =
      api_level >= 27 && provider_available(providers, "nnapi");
  backend_is_unhealthy(TRAPO_OCR_BACKEND_NNAPI, &nnapi.last_failure);
  if (api_level < 27) {
  ...
  capabilities.default_backend = TRAPO_OCR_BACKEND_CPU;
  for (const auto backend :
       {TRAPO_OCR_BACKEND_QNN, TRAPO_OCR_BACKEND_NNAPI,
        TRAPO_OCR_BACKEND_XNNPACK}) {
    const AcceleratorStatus* status = backend == TRAPO_OCR_BACKEND_QNN
                                          ? &qnn
  ...
  cpu.enabled_by_default = capabilities.default_backend == TRAPO_OCR_BACKEND_CPU;
  xnnpack.enabled_by_default =
      capabilities.default_backend == TRAPO_OCR_BACKEND_XNNPACK;
  nnapi.enabled_by_default =
      capabilities.default_backend == TRAPO_OCR_BACKEND_NNAPI;
  qnn.enabled_by_default =
      capabilities.default_backend == TRAPO_OCR_BACKEND_QNN;
  ...
  return capabilities;
    ...
    defined(TRAPO_OCR_ENABLE_DIRECTML)
    ...
    if (SUCCEEDED(adapter->GetDesc1(&description)) &&
      ...
      if (SUCCEEDED(device_result)) {
        ...
        if (unhealthy) {
          ...
          capabilities.default_backend = TRAPO_OCR_BACKEND_DIRECTML;
...
Ort::Env& ort_env() {
  static Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "trapo_ocr");
  return env;
}

bool backend_supported_for_ppocr(trapo_ocr_backend_t backend,
...
bool backend_supported_for_document_markdown(trapo_ocr_backend_t backend,
                                 std::string* unavailable_reason = nullptr) {
  if (backend == TRAPO_OCR_BACKEND_CPU) {
    return true;
  }
  if (backend != TRAPO_OCR_BACKEND_DIRECTML &&
    ...
    if (unavailable_reason != nullptr) {
      *unavailable_reason = backend_display_name(backend) +
                            " is not a Document Markdown ONNX backend.";
    }
    return false;
  }
  const RuntimeCapabilities capabilities = detect_runtime_capabilities();
...
std::vector<trapo_ocr_backend_t> ppocr_backend_attempts(
    const RuntimeOptions& runtime) {
  if (runtime.force_cpu_only || runtime.backend == TRAPO_OCR_BACKEND_CPU) {
    return {TRAPO_OCR_BACKEND_CPU};
  }
  if (runtime.backend != TRAPO_OCR_BACKEND_AUTO) {
    if (is_onnx_accelerator_backend(runtime.backend)) {
      return {runtime.backend, TRAPO_OCR_BACKEND_CPU};
    }
    return {TRAPO_OCR_BACKEND_CPU};
  }
...
std::vector<trapo_ocr_backend_t> document_markdown_backend_attempts(
    const RuntimeOptions& runtime) {
  if (runtime.force_cpu_only || runtime.backend == TRAPO_OCR_BACKEND_CPU) {
    return {TRAPO_OCR_BACKEND_CPU};
  }

  std::vector<trapo_ocr_backend_t> attempts;
  auto add_if_candidate = [&attempts](trapo_ocr_backend_t backend) {
  ...
  if (runtime.backend != TRAPO_OCR_BACKEND_AUTO) {
    if (runtime.backend == TRAPO_OCR_BACKEND_DIRECTML ||
        runtime.backend == TRAPO_OCR_BACKEND_CUDA) {
      add_if_candidate(runtime.backend);
    }
    add_if_candidate(TRAPO_OCR_BACKEND_CPU);
  } else {
    for (const auto backend :
         {TRAPO_OCR_BACKEND_CUDA, TRAPO_OCR_BACKEND_DIRECTML}) {
      std::string reason;
      if (backend_supported_for_document_markdown(backend, &reason)) {
  ...
  attempts.erase(
      std::remove_if(attempts.begin(), attempts.end(), [](auto backend) {
        return backend != TRAPO_OCR_BACKEND_CPU &&
               document_markdown_backend_is_unhealthy(backend);
      }),
      attempts.end());
  if (attempts.empty()) {
    attempts.push_back(TRAPO_OCR_BACKEND_CPU);
  }
  return attempts;
}

trapo_ocr_backend_t resolve_backend(const RuntimeOptions& runtime) {
  ...
  if (runtime.backend == TRAPO_OCR_BACKEND_AUTO) {
    const std::vector<trapo_ocr_backend_t> attempts =
        ppocr_backend_attempts(runtime);
    return attempts.empty() ? TRAPO_OCR_BACKEND_CPU : attempts.front();
  }
  return TRAPO_OCR_BACKEND_CPU;
}
...
class OrtRunner {
 public:
  OrtRunner(const std::string& model_path, const RuntimeOptions& runtime,
            const std::string& session_name, trapo_ocr_backend_t active_backend) {
    Ort::SessionOptions options;
    options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
    const int cpu_threads = effective_cpu_threads(runtime);
    if (active_backend == TRAPO_OCR_BACKEND_XNNPACK) {
...
#endif
    } else if (active_backend == TRAPO_OCR_BACKEND_XNNPACK) {
      const int xnnpack_threads = std::max(1, cpu_threads);
      options.AppendExecutionProvider(
          "XNNPACK",
          std::unordered_map<std::string, std::string>{
              {"intra_op_num_threads", std::to_string(xnnpack_threads)}});
    } else if (active_backend == TRAPO_OCR_BACKEND_NNAPI) {
#if defined(__ANDROID__) && defined(TRAPO_OCR_HAS_NNAPI_PROVIDER_FACTORY)
...
#endif
    } else if (active_backend == TRAPO_OCR_BACKEND_QNN) {
      options.AppendExecutionProvider(
...
cv::Mat decode_image(const trapo_ocr_image_t& image) {
...
#endif  // TRAPO_OCR_ENABLE_NATIVE_PIPELINE

}  // namespace

struct trapo_ocr_engine_t {
  trapo_ocr_pipeline_t pipeline = TRAPO_OCR_PIPELINE_PPOCRV6;
  ...
  trapo_ocr_run_options_t vl_defaults{};
  std::unique_ptr<trapo_ocr::PaddleOcrVlEngine> vl_native;
  trapo_ocr_run_options_t document_markdown_defaults{};
  std::unique_ptr<trapo_ocr::DocumentMarkdownEngine> document_markdown_native;
#endif
};
...
bool is_ort_or_backend_failure_message(trapo_ocr_backend_t backend,
                                       const std::string& message) {
  if (backend == TRAPO_OCR_BACKEND_DIRECTML) {
...
bool runtime_allows_onnx_backend_fallback(const RuntimeOptions& runtime) {
  return !runtime.force_cpu_only && runtime.backend != TRAPO_OCR_BACKEND_CPU;
}

bool should_retry_ppocr_create(const RuntimeOptions& runtime,
                               trapo_ocr_backend_t failed_backend,
                               const std::string& message) {
  return runtime_allows_onnx_backend_fallback(runtime) &&
         failed_backend != TRAPO_OCR_BACKEND_CPU &&
         is_ort_or_backend_failure_message(failed_backend, message);
}

bool should_retry_ppocr_recognition(const trapo_ocr_engine_t& engine,
                                    const std::string& message) {
  if (!runtime_allows_onnx_backend_fallback(engine.requested_runtime) ||
      !engine.native ||
      engine.native->active_backend() == TRAPO_OCR_BACKEND_CPU) {
    return false;
  }
...
std::vector<trapo_ocr_backend_t> remaining_ppocr_attempts_after_failure(
    const RuntimeOptions& runtime, trapo_ocr_backend_t failed_backend) {
  ...
  if (runtime.backend != TRAPO_OCR_BACKEND_AUTO) {
    return failed_backend == TRAPO_OCR_BACKEND_CPU
               ? std::vector<trapo_ocr_backend_t>{}
               : std::vector<trapo_ocr_backend_t>{TRAPO_OCR_BACKEND_CPU};
  }
  ...
  for (auto it = failed + 1; it != priority.end(); ++it) {
    if (*it == TRAPO_OCR_BACKEND_CPU || backend_supported_for_ppocr(*it)) {
      out.push_back(*it);
    }
  }
  return out;
...
std::vector<trapo_ocr_backend_t> remaining_document_markdown_attempts_after_failure(
    const RuntimeOptions& runtime, trapo_ocr_backend_t failed_backend) {
  if (!runtime_allows_onnx_backend_fallback(runtime)) {
    return {};
  }
  ...
  std::vector<trapo_ocr_backend_t> out;
  for (auto it = failed + 1; it != priority.end(); ++it) {
    if (*it == TRAPO_OCR_BACKEND_CPU || backend_supported_for_document_markdown(*it)) {
      out.push_back(*it);
    }
  }
  return out;
...
RuntimeOptions runtime_for_backend(const RuntimeOptions& requested,
  ...
  if (backend == TRAPO_OCR_BACKEND_CPU) {
    ...
    runtime.generative_backend = TRAPO_OCR_GEN_BACKEND_CPU;
    runtime.generative_gpu_layers = 0;
  }
  return runtime;
}

std::string ppocr_fallback_warning(trapo_ocr_backend_t failed_backend,
                                   trapo_ocr_backend_t retry_backend,
                                   const std::string& message) {
...
trapo_ocr_status_t trapo_ocr_get_runtime_capabilities(
    trapo_ocr_result_t** out_result) {
  if (out_result == nullptr) {
    return fail(TRAPO_OCR_INVALID_ARGUMENT, "out_result is null");
  }
  *out_result = nullptr;
  try {
    ...
    *out_result = allocate_result(capabilities_json(capabilities));
    ...
    return fail(TRAPO_OCR_INTERNAL_ERROR, error.what());
  }
}

trapo_ocr_status_t trapo_ocr_create(const trapo_ocr_init_options_t* options,
                                  trapo_ocr_engine_t** out_engine) {
  if (out_engine == nullptr) {
    return fail(TRAPO_OCR_INVALID_ARGUMENT, "out_engine is null");
  }
  *out_engine = nullptr;
  if (options == nullptr ||
      !has_size(options->struct_size, sizeof(trapo_ocr_init_options_t))) {
    return fail(TRAPO_OCR_INVALID_ARGUMENT, "invalid init options");
  }
  if (options->model_root == nullptr || options->model_root[0] == '\0') {
    return fail(TRAPO_OCR_INVALID_ARGUMENT, "model_root is required");
  }
  if (!has_size(options->runtime.struct_size, kRequiredRuntimeOptionsSize) ||
    ...
    return fail(TRAPO_OCR_INVALID_ARGUMENT, "invalid nested init options");
  }

  try {
    const trapo_ocr_pipeline_t pipeline = options->pipeline;
    if (pipeline == TRAPO_OCR_PIPELINE_DOCUMENT_MARKDOWN) {
      const std::string bundle_root =
          options->external_model_root != nullptr &&
              ...
              : options->model_root;
      const trapo_ocr::DocumentMarkdownBundleCheck bundle =
          trapo_ocr::ValidateDocumentMarkdownBundle(bundle_root);
      if (!bundle.ok) {
        return fail(TRAPO_OCR_UNAVAILABLE, bundle.message());
      }
#if defined(_WIN32) && defined(TRAPO_OCR_ENABLE_NATIVE_PIPELINE)
      auto engine = std::make_unique<trapo_ocr_engine_t>();
      ...
      engine->document_markdown_defaults = run_options_to_c(engine->defaults);
      const std::vector<trapo_ocr_backend_t> attempts =
          document_markdown_backend_attempts(engine->requested_runtime);
      std::vector<std::string> failures;
      for (trapo_ocr_backend_t backend : attempts) {
        RuntimeOptions attempt_runtime =
            runtime_for_backend(engine->requested_runtime, backend);
        try {
          engine->document_markdown_native =
              std::make_unique<trapo_ocr::DocumentMarkdownEngine>(
          ...
          if (!should_retry_ppocr_create(engine->requested_runtime, backend,
                                         failure)) {
            if (backend != TRAPO_OCR_BACKEND_CPU) {
              mark_document_markdown_backend_unhealthy(backend, failure);
            }
            throw;
          }
          mark_document_markdown_backend_unhealthy(backend, failure);
      ...
      if (!engine->document_markdown_native) {
        std::ostringstream message;
        message << "Document Markdown initialization failed for all runtime "
                   "backends.";
        if (!failures.empty()) {
          message << " Attempts: ";
        ...
        return fail(TRAPO_OCR_INTERNAL_ERROR, message.str());
      }
      *out_engine = engine.release();
      g_last_error.clear();
      return TRAPO_OCR_OK;
#else
      return fail(TRAPO_OCR_UNAVAILABLE,
                  "Document Markdown ONNX pipeline is Windows-only in this build.");
#endif
    }

    if (pipeline == TRAPO_OCR_PIPELINE_PADDLEOCR_VL16) {
      const std::string bundle_root =
          options->external_model_root != nullptr &&
                  options->external_model_root[0] != '\0'
              ? options->external_model_root
              : options->model_root;
      const trapo_ocr::PaddleOcrVlBundleCheck bundle =
          trapo_ocr::ValidatePaddleOcrVl16Bundle(
              bundle_root,
...
#if defined(TRAPO_OCR_ENABLE_NATIVE_PIPELINE)
      if (!trapo_ocr::LlamaVlmRuntimeAvailable()) {
        return fail(TRAPO_OCR_UNAVAILABLE,
                    trapo_ocr::LlamaVlmUnavailableReason());
      }
      auto engine = std::make_unique<trapo_ocr_engine_t>();
      engine->pipeline = pipeline;
      engine->model_root = options->model_root;
      ...
      const trapo_ocr_runtime_options_t sanitized_runtime =
          runtime_options_to_c(engine->runtime);
      engine->vl_native =
          std::make_unique<trapo_ocr::PaddleOcrVlEngine>(bundle,
                                                        sanitized_runtime);
      *out_engine = engine.release();
      g_last_error.clear();
      return TRAPO_OCR_OK;
...
trapo_ocr_status_t trapo_ocr_recognize_image(trapo_ocr_engine_t* engine,
  ...
  *out_result = nullptr;
  ...
  try {
#if defined(TRAPO_OCR_ENABLE_NATIVE_PIPELINE)
    if (engine->pipeline == TRAPO_OCR_PIPELINE_DOCUMENT_MARKDOWN) {
      if (!engine->document_markdown_native) {
        return fail(TRAPO_OCR_UNAVAILABLE,
                    "Document Markdown runtime is not initialized");
      }
      trapo_ocr_run_options_t document_markdown_options =
          options == nullptr ? engine->document_markdown_defaults : run_options_to_c(resolved);
      try {
        *out_result = allocate_result(
            engine->document_markdown_native->Recognize(*image, document_markdown_options));
      } catch (const std::exception& error) {
        const std::string backend_error = error.what();
        const trapo_ocr_backend_t failed_backend =
            engine->document_markdown_native->active_backend();
        if (!runtime_allows_onnx_backend_fallback(engine->requested_runtime) ||
            failed_backend == TRAPO_OCR_BACKEND_CPU ||
            !is_ort_or_backend_failure_message(failed_backend, backend_error)) {
          if (failed_backend != TRAPO_OCR_BACKEND_CPU &&
              is_ort_or_backend_failure_message(failed_backend, backend_error)) {
            mark_document_markdown_backend_unhealthy(failed_backend, backend_error);
          }
          throw;
        }
        ...
        for (trapo_ocr_backend_t retry_backend :
             remaining_document_markdown_attempts_after_failure(engine->requested_runtime,
                                                    failed_backend)) {
          ...
          try {
            ...
            *out_result = allocate_result(engine->document_markdown_native->Recognize(
                *image, document_markdown_options, fallback_warnings));
            break;
          } catch (const std::exception& retry_error) {
            const std::string retry_message = retry_error.what();
            retry_failures.push_back(backend_display_name(retry_backend) + ": " +
                                     retry_message);
            if (retry_backend != TRAPO_OCR_BACKEND_CPU) {
              mark_document_markdown_backend_unhealthy(retry_backend, retry_message);
            }
          }
        }
        if (*out_result == nullptr) {
          ...
          message << backend_display_name(failed_backend)
                  << " failed and all automatic Document Markdown retry backends "
                     "failed.";
    ...
    if (engine->pipeline == TRAPO_OCR_PIPELINE_PADDLEOCR_VL16) {
      if (!engine->vl_native) {
        return fail(TRAPO_OCR_UNAVAILABLE,
                    "PaddleOCR-VL runtime is not initialized");
      }
      trapo_ocr_run_options_t vl_options =
    ...
    if (!engine->native) {
      return fail(TRAPO_OCR_UNAVAILABLE, "native OCR runtime is not initialized");
    }
    try {
    ...
    const std::string message =
        "native OCR core was built without ONNX Runtime/OpenCV support";
    *out_result = allocate_result(
        make_error_json(TRAPO_OCR_UNAVAILABLE, message, "native-stub"));
    return TRAPO_OCR_UNAVAILABLE;
#endif
  } catch (const std::invalid_argument& error) {
    return fail(TRAPO_OCR_INVALID_ARGUMENT, error.what());
  } catch (const std::exception& error) {
    return fail(TRAPO_OCR_INTERNAL_ERROR, error.what());
  }
}

void trapo_ocr_free_result(trapo_ocr_result_t* result) {
  if (result == nullptr) {
    return;
  }
...
void trapo_ocr_destroy(trapo_ocr_engine_t* engine) { delete engine; }

const char* trapo_ocr_last_error(void) { return g_last_error.c_str(); }

const char* trapo_ocr_engine_runtime_summary(trapo_ocr_engine_t* engine) {
  ...
  if (engine->pipeline == TRAPO_OCR_PIPELINE_DOCUMENT_MARKDOWN &&
      engine->document_markdown_native) {
    summary = engine->document_markdown_native->runtime_summary();
    return summary.c_str();
  }
  if (engine->pipeline == TRAPO_OCR_PIPELINE_PADDLEOCR_VL16 &&
      engine->vl_native) {
