...
cv::Mat DecodeImage(const trapo_ocr_image_t& image) {
...
std::pair<int, int> AspectRatioPreservingSize(int height, int width,
  ...
  if (target_h == 0 && target_w == 0) {
    throw std::runtime_error("Document Markdown image resize target became 0x0");
  }
  const int max_side =
...
void Patchify(const cv::Mat& rgb, int max_soft_tokens,
              DocumentMarkdownImageInputs* output) {
  const int max_patches =
      max_soft_tokens * kPoolingKernelSize * kPoolingKernelSize;
...
void EncodeOverlay(const cv::Mat& image, DocumentMarkdownImageInputs* output) {
  ...
  if (cv::imencode(".jpg", image, encoded, jpeg_params)) {
    ...
    output->overlay_image_base64 = DocumentMarkdownBase64Encode(encoded);
    return;
  }
  ...
  throw std::runtime_error("failed to encode DocumentMarkdown preview image");
}
...
DocumentMarkdownImageProcessor::DocumentMarkdownImageProcessor(
    ...
    : doc_orientation_(bundle.doc_orientation_model_path, backend, cpu_threads,
                       enable_profiling, "document_markdown_doc_orientation") {}

int DocumentMarkdownImageProcessor::DetectOrientation(const cv::Mat& page) const {
  cv::Mat resized = ResizeShortAndCrop(page, 256, 224);
  auto input = NormalizeToChw(resized);
  if (doc_orientation_.inputs().empty()) {
    throw std::runtime_error("DocumentMarkdown orientation model has no inputs");
  }
  auto outputs = doc_orientation_.Run(
      {DocumentMarkdownTensor::Float(doc_orientation_.inputs().front().name,
...
DocumentMarkdownImageInputs DocumentMarkdownImageProcessor::Process(
  ...
  if (use_doc_orientation) {
    ...
    DocumentMarkdownLogInfo("core document markdown doc orientation angle=" +
                 std::to_string(output.doc_angle) + " elapsedMs=" +
                 std::to_string(DocumentMarkdownElapsedMs(
