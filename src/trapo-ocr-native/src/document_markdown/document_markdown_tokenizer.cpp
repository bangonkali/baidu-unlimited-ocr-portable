...
DocumentMarkdownTokenizer::DocumentMarkdownTokenizer(const std::string& tokenizer_json_path) {
...
int64_t DocumentMarkdownTokenizer::TokenId(const std::string& token) const {
...
bool DocumentMarkdownTokenizer::IsEos(int64_t token_id) const {
  return eos_token_ids_.find(token_id) != eos_token_ids_.end();
}

std::vector<std::string> DocumentMarkdownTokenizer::SegmentToPieces(
  ...
  for (size_t i = 0; i < segment.size();) {
    ...
    if (vocab_.find(piece) != vocab_.end()) {
      ...
      for (size_t b = 0; b < length; ++b) {
        pieces.push_back(
            DocumentMarkdownTokenizerByteToken(static_cast<unsigned char>(segment[i + b])));
      }
    }
    i += length;
  }
...
std::vector<int64_t> DocumentMarkdownTokenizer::EncodeTextSegment(
    const std::string& segment) const {
  ...
  while (pieces.size() > 1) {
    ...
    for (size_t i = 0; i + 1 < pieces.size(); ++i) {
      const auto found =
          merge_ranks_.find(DocumentMarkdownTokenizerMergeKey(pieces[i], pieces[i + 1]));
...
std::vector<int64_t> DocumentMarkdownTokenizer::Encode(const std::string& text) const {
  std::vector<int64_t> ids;
  std::string normalized;
  normalized.reserve(text.size());
  for (char c : text) {
    if (c == ' ') {
      normalized.append(kDocumentMarkdownSpaceToken);
    } else {
...
std::string DocumentMarkdownTokenizer::Decode(const std::vector<int64_t>& token_ids,
  ...
  for (int64_t id : token_ids) {
    ...
    if (DocumentMarkdownTokenizerIsByteToken(token, &byte_value)) {
      pending_bytes.push_back(byte_value);
      continue;
    }
    flush_bytes();
    size_t pos = 0;
    while ((pos = token.find(kDocumentMarkdownSpaceToken, pos)) != std::string::npos) {
      token.replace(pos, std::char_traits<char>::length(kDocumentMarkdownSpaceToken),
                    " ");
      ++pos;
    }
    out.append(token);
  ...
  flush_bytes();
  return DocumentMarkdownTrim(out);
}
