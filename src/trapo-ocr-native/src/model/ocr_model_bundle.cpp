...
std::string DocumentMarkdownBundleCheck::summary() const {
  std::ostringstream out;
  out << "Document Markdown ONNX bundle";
  if (!root.empty()) {
    out << " root=" << root;
  }
  return out.str();
...
std::string DocumentMarkdownBundleCheck::message() const {
  if (ok) {
    return "Document Markdown bundle is present.";
  }
  std::ostringstream out;
  out << "Document Markdown model bundle is incomplete.";
  if (!missing.empty()) {
    out << " Missing:";
    for (const std::string& path : missing) {
      out << " " << path;
...
DocumentMarkdownBundleCheck ValidateDocumentMarkdownBundle(const std::string& root) {
  DocumentMarkdownBundleCheck check;
  check.root = root;
  check.manifest_path = JoinPath(root, "manifest.json");
