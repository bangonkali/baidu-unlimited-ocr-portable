...
describe('realtime events', () => {
  ...
  test('stores document text and region payloads by file hash and run id', () => {
    const client = new QueryClient();
    applyRealtimeEventToQueryClient(client, {
    ...
    expect(client.getQueryData(queryKeys.documentRegions('abc', 'run-a'))).toEqual({
      boxes: [
        {
          height_percent: 4,
          label: 'Total',
    ...
    expect(client.getQueryData(queryKeys.documentText('abc', 'run-a'))).toEqual({
      file_hash: 'abc',
      pages: [
        { page_no: 1, spans: [{ end: 5, page_no: 1, region_id: 'r1', start: 0 }], text: 'Total' },
      ],
